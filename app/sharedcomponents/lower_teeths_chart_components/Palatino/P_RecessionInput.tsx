...
const P_RecessionInput = (props: Props) => {
  ...
  const handleInputChange = (value: string, inputIndex: number) => {
    ...
    if (value === "-" || value === "") {
      ...
      if (
        ...
        if (inputIndex < inputRefs.current.length - 1) {
        ...
        } else if (props.index < props.recessionData.length - 1) {
          ...
          const nextObjectFirstInput = document.getElementById(
            `inputrecessionlowerbottom-${props.index + 1}-0`
          );
  ...
  return (
    <div className="flex items-center">
      {inputValues.map((item: string, index: number) => (
        <input
          key={index}
          id={`inputrecessionlowerbottom-${props.index}-${index}`}
          type="number"
          value={item}
          ref={(el) => {
...
export default P_RecessionInput;
